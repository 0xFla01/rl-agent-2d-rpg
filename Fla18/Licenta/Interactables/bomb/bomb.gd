...
func _ready() -> void:
	...
	hurt_box.damage = 0
	# Bug fix: bomb.tscn are 2 HurtBox-uri — Throwable/HurtBox (wall detect, damage=0)
	# si Bomb/HurtBox (explozie, folosit de track-ul de monitoring din anim "explode").
	# Cel din scene avea damage=1 (HurtBox default), de aceea bombele faceau 1 dmg.
	var explode_hb := prop.get_node_or_null( "HurtBox" ) as HurtBox
	if explode_hb:
		explode_hb.damage = 20
